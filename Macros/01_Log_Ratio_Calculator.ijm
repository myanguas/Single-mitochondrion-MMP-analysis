 ...
 *  Fluidity Calculator v2.0.4 adapted to calculate log-ratio of 2 channels instead of GP
 *  
 *  PCNOTES:
 *  	- IDEA: Implement double thresholding two effectively mask both mitochondrial types
 *  	- Change icon
 *  	- Change 'gp' to 'log-ratio'
 *  
 *  ----------------------------------------------------------------------------------
 *  Author: Pablo Carravilla
 *  
 *  Name: Log-Ratio Calculator
 *  Version: 0.2.0.4
 *  Date: 30-10-2024
 *  
 *  Improved and updated by: Asier Ramos & Martin Yanguas
 *  Description: 
 *  This macro tool calculates Log-Ratio images from multichannel microscopy images.
 *  
 *  
...
macro "Log Ratio Calculator v0.2.0.4 Action Tool - icon:fluidity.png" {
	...
	print(" ");
	print("Running Log-Ratio Calculator");
	...
	if (saveResultsUser == 1 || saveAnalysisMaskUser == 1) {
    	...
    	// Carpeta base Results dentro del directorio de la imagen
    	saveDirUser = imgDir + "Results" + File.separator;

    	// Comprueba si la carpeta existe, si no, la crea
    	if (!File.exists(saveDirUser)) {
        	File.makeDirectory(saveDirUser);
        	print("Carpeta 'Results' creada en: " + saveDirUser);
    	} else {
        	print("Carpeta 'Results' ya existe en: " + saveDirUser);
    	}
	...
	// Create saturation mask (negative, i.e. saturated pixels = 0)
	// It will be used to mask the Log-Ratio image
	...
	// Calculate Log-Ratio from channels 1 and 2
	calculateLogRatio(ch1ID, ch2ID);
	rename("Log-Ratio_" + nameDate);
	gpID = getImageID();
	
	
	
	// Calculate and save (optional) analysis mask (pixels within threshold that are not saturated)
	imageCalculator("AND 32-bit create stack", thrMaskID, gpSatMaskID);
	analysisMaskID = getImageID();
	if (saveAnalysisMaskUser == 1) {saveAs("tiff", saveDirUser + "Log-Ratio_Analysis_Mask_" + nameDate);}
	
	
	// Mask using analysis (threshold AND saturation) mask
	...
	// Histogram
	setMinAndMax(histoMin, histoMax); // Comment out to use automatic range
	run("Histogram", "bins=50 x_min=histoMin x_max=histoMax y_max=Auto");
	...
	run("Tile");
	print("Finished running Log-Ratio Calculator");
	...
	// or a negative mask, where saturated pixels = 0
		...
		for (c = 1; c <= dataImgChannels; c++) {
			// Check if the channel is in the range defined by the user for Log-Ratio calculation
	...
	// Extracts channels selected by user for Log-Ratio calculation
	...
	function calculateLogRatio(ch1Img, ch2Img) {
	// Calculates Log-Ratio image from ch1 and ch2 after thresholding
		...
		imageCalculator("Divide create 32-bit stack", denominatorImg, numeratorImg);
		run("Log");
		...
		selectImage(gpImg); 
		// Output image is Log-Ratio image	
	...
	// and a threshold mask as a reference, 
		...
		if (satPerc >=  warningPerc && showWarning == 1) {
			...
			Dialog.addMessage("Attention!\n"
		            ...
		            + "These pixels will not be used for Log-Ratio quantification.");
	...
	// Generates and saves (optional) a table containing Log-Ratio and channel statistics
		
		selectImage(gpImg);
		getDimensions(gpImgWidth, gpImgHeight, gpImgChannels, gpImgSlices, gpImgFrames);
		
		// Create table
		gpTableName = "Log-Ratio_Results_" + saveName;
	...
	// Saves Log-Ratio histogram data
		...
		for (f = 1; f <= gpHistoFrames; f++) { // First frames
			for (z = 1; z <= gpHistoSlices; z++) {
				...
				// PCNOTES THIS PROBABLY WON'T WORK, AS RATIO CAN TAKE ANY VALUE
				getHistogram(gpValues, gpCounts, binNumber, -6, 6); // gpValues and gpCounts are arrays
				
				// Values (only first column)
				if (f == 1 && z == 1) {Table.setColumn("Log-Ratio Values", gpValues);} // Bin values only the first time
	...
	// Shows GUI with default settings
		...
		Dialog.create("Log-Ratio Calculator");
		...
		saveLocationList = newArray("Image Directory", "Image Directory/Log-Ratio_Results", "Select");
